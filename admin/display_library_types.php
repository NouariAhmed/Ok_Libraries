...
$result = mysqli_query($conn, "SELECT * FROM library_types LIMIT $startIndex, $itemsPerPage");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['submit'])) {
        ...
        if (empty($library_type)) {
            ...
            $stmt = mysqli_prepare($conn, "INSERT INTO library_types (library_type) VALUES (?)");
            mysqli_stmt_bind_param($stmt, "s", $library_type);
...
?>
    ...
    // Check if create_update_success session variable is set
        ...
        <h4 class="mb-3">إضـافة نوع مكتبة</h4>
          <div class="input-group input-group-outline my-3">
            <label class="form-label">نوع المكتبة :</label>
              <input type="text" name="library_type" class="form-control" value="<?php echo htmlspecialchars(isset($item['library_type']) ? $item['library_type'] : ''); ?>" required>
              </div>
          ...
          <div class="card my-4">
            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
              <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                <h6 class="text-white text-capitalize pe-3">جدول أنواع المكتبات</h6>
              </div>
            </div>
                ...
                <table class="table align-items-center mb-0">
                  <thead>
                    <tr>
                      ...
                      <th class="text-secondary text-lg font-weight-bolder opacity-7 pe-2">نوع المكتبة</th>
...
?>
                    <tr>
                      ...
                      <h6 class="mb-0 text-sm"><?php echo $library_type;?></h6>
                      </td>
                      <td class="align-middle text-center">
                            <a href="update_library_type.php?id=<?php echo $id;?>" class="btn badge-sm bg-gradient-primary"> <i class="material-icons-round align-middle" style="font-size: 18px;">edit</i></a>
                            <a href="delete_library_type.php?id=<?php echo $id;?>" class="btn badge-sm bg-gradient-danger"> <i class="material-icons-round align-middle" style="font-size: 18px;">delete</i></a>                          
                      </td>
                    </tr>
                    <?php
                }
